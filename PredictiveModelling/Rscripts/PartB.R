  ...
  set.seed(3141)
    #strictly speaking we don't need to set the seed, this data is a 5th degree
      ...
      # the crosshairs are always drawn at the point of lowest validation error.
          #Although ocassionaly the validation error will be slightly lower
          #for more complex models, it can be good to adjust
          #the seed such that the crosshairs do line up.
...
# Plot the Errors
      ...
      min(diff(error.df$`Validation Error`))
 ...
 ##Make a slightly smoother plot
  ...
  all.plot +
    ...
    scale_color_brewer(palette="Set3",
                       labels = c("3rd Degree Polynomial Model",
                                  "Test Set",
                                  "Training Set",
                                  "Validation Set")) +
    guides(col = guide_legend(title = NULL)) +
