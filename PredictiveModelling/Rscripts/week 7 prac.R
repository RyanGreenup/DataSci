...
##Build models
fit <- glm(Survived ~ Age + Pclass, family = binomial(link='logit'),data=mydata)
fit1 <- glm(Survived ~ I(Age>13) + Age + Pclass, family = binomial(link='logit'),data=mydata)
fit2 <- glm(Survived ~ I(Age^2) + Age + Pclass, family = binomial(link='logit'),data=mydata)

##Select best model
aics <- c(summary(fit)$aic, summary(fit2)$aic, summary(fit2)$aic)

bestfit <- switch(EXPR = which.min(aics), fit,fit1,fit2)
print("Best fit: ")
print(bestfit$call)
...
for(i in 1:steps){
  ...
  roc[i, "tpr"] <- sensitivity(pre, mydata$Survived, thresholds[i])
  roc[i, "sum"] <- specificity(pre, mydata$Survived, thresholds[i]) + sensitivity(pre, mydata$Survived, thresholds[i])
}
