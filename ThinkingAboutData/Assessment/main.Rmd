...
sydney_mod <- lm(Sydney ~ date, data)
sydney_slope <- sydney_mod$coefficients["date"]

melbourne_mod <- lm(Melbourne ~ date, data)
melbourne_slope <- melbourne_mod$coefficients["date"]
...
## Plot
In order to plot the data it is necessary to selec

```{r}
ggplot(data %>% 
         filter(city %in% c("Sydney", "Melbourne")), aes(x = date, y = newTiara, col = city)) +
  geom_point()
```

## Observations from Plot
## Analysis and Resultse
## Conclusion
