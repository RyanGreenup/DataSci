...
```{r}
mean_city <- aggregate(newFlu ~ city, mean_city, mean, na.rm = TRUE)
...
```{r}
ggplot(mean_city, aes(x = city, y = newFlu, fill = city)) +
  geom_col() +
  theme_classic() +
...
The statistic of concern will be the probability of a Type I error ($\alpha$) which is
...
any difference between mean values, then the 
