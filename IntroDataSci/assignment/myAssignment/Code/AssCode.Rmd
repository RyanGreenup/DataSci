...
So instead we will consider the inverse of Cycle Time in order to get a linear model, more over it stands to reason the cpu performanc will by proportional to the frequency of the cpu rather than the period, further evidence justifying this transformation.

```{r}
cpu_invCyc <- cpu
cpu_invCyc$CycleTime <- cpu$CycleTime^-1
names(cpu_invCyc)[1] <- "freq"
...
```{r}
cpu.tree <- tree(Performance ~ ., data = cpu_invCyc)
plot(cpu.tree)
...
You can't plot residuals of functions that have an I() term [^stack] in the response
...
```{r}
cpu_invCyc_LogPerf <- cpu_invCyc
cpu_invCyc_LogPerf$Performance <- log(cpu$Performance)
cpu.loglm <- lm(formula = Performance ~ . +I(MaximumMainMemory * MaximumNumberOfChannels), data = cpu_invCyc_LogPerf)
summary(cpu.loglm)
...
Minimum numer of Channels is the least significant and it is strongly correlated with maximum Number of Channels and so it is dropped


```{r}
cpu_invCyc_LogPerf <- cpu_invCyc
cpu_invCyc_LogPerf$Performance <- log(cpu$Performance)
cpu.loglm <- lm(formula = Performance ~ . -MinimumNumberOfChannels +I(MaximumMainMemory * MaximumNumberOfChannels), data = cpu_invCyc_LogPerf)
summary(cpu.loglm)
...
Minimum Main Memory is not significant, it is also strongly correlated with Maximum main memory and so we elect to remove it from our model (i.e. Max will predict min so it only serves to over parameterise)
...
cpu_invCyc_LogPerf <- cpu_invCyc
cpu_invCyc_LogPerf$Performance <- log(cpu$Performance)
cpu.loglm <- lm(formula = Performance ~ . -MaximumMainMemory -MinimumNumberOfChannels +I(MaximumMainMemory * MaximumNumberOfChannels), data = cpu_invCyc_LogPerf)
summary(cpu.loglm)
...
The interaction term is no longer significant and so it is removed:


```{r}
cpu_invCyc_LogPerf <- cpu_invCyc
cpu_invCyc_LogPerf$Performance <- log(cpu$Performance)
cpu.loglm <- lm(formula = Performance ~ . -MaximumMainMemory -MinimumNumberOfChannels , data = cpu_invCyc_LogPerf)
summary(cpu.loglm)
...
All terms are significant and the residuals are sufficiently normal, however there appears to be pattern in the residuals, for this we may consider transforming the predictor variables, we will consider this in the next question by fitting polynomial models.
