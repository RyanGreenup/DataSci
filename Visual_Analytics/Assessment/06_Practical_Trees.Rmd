...
```{r}
oec <- read.csv(file = "./oec.csv", header = TRUE, sep = ",")
str(oec)
oec$ListsPlanetIsOn %>% levels

# Only use features with more than 50% values
f <- function(x) mean(is.na(x)) 
navals <- apply(oec, 2, f) > 0.5
keep_feat <- navals[navals > 0.5] %>% names()
oec <- dplyr::select(oec, all_of(keep_feat))

# Make Na Mean
# easiest is zoo::na.aggregate()
na_mean <- function(x) {
  replace(x, is.na(x), mean(x, na.rm = TRUE))
}
na_mean <- replace(oec, TRUE, lapply(oec, na_mean))


oec.tree <- tree(formula = ListsPlanetIsOn ~ ., data = as.data.frame(oec), na.action = na.)
?tree
tree()
