...
def readWriteFile(infile, outfile):
    ...
    return returnMatchingData(outFile)

def readTheTextFile(infile):
def returnMatchingData(infile):
...
for values in obs:
    ## Don't Require whitespace after comma
    course_name = values[1].replace(', ',',').split(',')[3-1]
    unit_name   = values[1].replace(', ',',').split(',')[2-1]
    if course_name == "MICT":
        matches.append(values[0])

[ obs[i][1].replace(', ', ',').split(',')[2-1] for i in matches ]
