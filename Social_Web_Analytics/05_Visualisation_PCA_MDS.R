...
pol.pca <- prcomp(tweet_weighted, scale = TRUE)
...
pcaVarpr$Principal.Component <- factor(pcaVarpr$Principal.Component, ordered = TRUE, levels = pcaVarpr$Principal.Component)
...
ggplot(data = pcaVarpr, aes( x = Principal.Component, y = Proportion.Variance, group = 1)) +
  ...
  labs(x = "Principal Component", y = "Proportion of Variance", title = "Variance Explained by PC, TF-IDF Weigthing Tweets by Party Leaders") +
  theme_classic() +
  geom_vline(xintercept = 4, col = "purple", lty = 2)

# Build the Plot ==============================================================
pca_data        <- data.frame(pol.pca$x[,1:4])
pca_data$Party  <- factor(c(rep("Greens", n), rep("Labor", n), rep("Liberal", n)))
pol.km <- kmeans(tweet_weighted, centers = 3, nstart = 200) %>%  as.vector()
pca_data$kmeans <- as.factor(pol.km$cluster)
head(pca_data)

ggplot(data = pca_data, aes(x = PC1, y = PC2, col = Party, shape= kmeans)) +
  geom_point() + 
  stat_ellipse()

# Using Clustering #############################################################
pca_dist <- dist(pca_data)              # Calculate Euclidean Distance
pca_dist <- dist(tweet_weighted)              # Calculate Euclidean Distance
hc_pca   <- hclust(pca_dist,                  # Perform Heirarchical 
                   method = "average")       #         Clustering
hc_pred  <- cutree(hc_pca, k = 3)           # Choose 3 groups
      hc_pred              
      
 # Use pca$x
 # create a new df PC1, PC2, PC3, Party, Clustered Allocation
   # Try to visualise how accurate the clustering is if possible using ellipses.
